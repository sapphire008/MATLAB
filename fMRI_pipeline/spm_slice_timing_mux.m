function spm_slice_timing_mux(P, sliceorder, refslice, timing, prefix)
% Correct differences in slice acquisition times
% FORMAT spm_slice_timing(P, sliceorder, refslice, timing, prefix)
% P           - nimages x ? Matrix with filenames. Needs to be character
%               array instead of cellstr
...
%               temporal acquisition order. This is usually a 1 x N vector.
%               In case of multiplex acquisition, input as a M x N matrix,
%               so that each row of the matrix represents a slab that are
%               acquired in parallel with other slabs/ rows. Make sure
%               there is no repeated labeling, and the number of elements
%               of sliceorder equals to the slice number.
% refslice    - slice for time 0. Input either as a integer or as a vector
%               of legnth M, so that each element of refslice corresponds
%               to each row of sliceorder. If sliceorder has multiple rows
%               whereas refslice has only one element, all rows of
%               sliceorder will use the same reference slice.
...
%
%   It has been verified the reslicing image acquired by multiplex parallel
%   image acquisition using the current function is the same as separating 
%   the slices into multiple slabs in parallel, then do slice timing on 
%   each individual slabs as if independent volumes.
%
...
%
% Modified by E.D. Cui for slice timing of multiplex parallel image
% acquisition
%__________________________________________________________________________
...
[Finter,Fgraph,CmdLine] = spm('FnUIsetup','SliceTiming',0);
...
if iscellstr(P) && numel(P)>1%cellstr array of files
    P = {char(P)};
...
Vin     = spm_vol(P{1}(1,:));%get the first image for reference parameters
nslices = Vin(1).dim(3);%number of slices

if nargin < 5, prefix = 'a'; end

% check to see if number of reference slice is the same as the number
% of rows in sliceorder

if numel(refslice) ~= size(sliceorder,1)
    error(['Number of reference slice is not consistent with',...
        ' number of rows in slice order']);
end

%calculate timing parameters: in case of multiplex, it has to rely on the
%size of the second dimension of sliceorder
TR  = (size(sliceorder,2)-1)*timing(1)+timing(2);
...
for subj = 1:nsubjects
    ...
    % slice timing
    for M = 1:size(sliceorder,1)
        Vout = reslice_timing_main_func(Vin,Vout,refslice(M),...
            sliceorder(M,:),factor,nimg,nimgo);
    end
...
function Vout = reslice_timing_main_func(Vin,Vout,refslice,sliceorder,...
    factor,nimg,nimgo)
% Set up large matrix for holding image info
% Organization is time by voxels
slices = zeros([Vout(1).dim(1:2), nimgo]);%current slice of all volumes
stack  = zeros([nimg, Vout(1).dim(1)]);

% find reference slice index
...
for k = min(sliceorder):1:max(sliceorder)
    ...
    % write out the slice for current slab of all volumes
    for p = 1:nimgo
