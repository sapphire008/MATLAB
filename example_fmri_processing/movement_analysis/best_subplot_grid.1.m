function [row,col]=best_subplot_grid(num_plots)

%if the number of plots is a primer number
if isprime(num_plots) || any(diff(factor(num_plots))>4)
    row = ceil(sqrt(num_plots));
    col = floor(sqrt(num_plots));
%if the number is a composite number
else
    prime_factor_num = factor(num_plots);
    [~,~,~,IND]=absExtreme(prime_factor_num-sqrt(num_plots));
    row = prime_factor_num(IND);
    col = num_plots/row;
end

%if there is only one input, put row and colum together
if nargout<2
    row = [row,col];
end
end
