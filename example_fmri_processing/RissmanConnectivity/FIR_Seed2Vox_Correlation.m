function FIR_Seed2Vox_Correlation(SPM,Events,ROI_loc,ROI_names,Mask_loc,output_image,postproc,verbose)
...
P = [];
for bf = 2%1:SPM.xBF.order
    % basis x trial
    P = [P;location_of_beta_images_from_event_discription(...
        {Events,sprintf('bf(%d)',bf)},SPM,true)];
end
if isempty(P)
    V_map = [];
...
for s = 1:dim(3)
    ...
    if all(~Mask_Y(:))
        ...
        %Image_Y = block_AUC(Image_Y,1,SPM.xBF.order);
        image_type = 'postproc';
    end
    clear Mask_Y;
