function [BATCH] = native_connectivity(P,S,M,ROI,R,V,save_conn,varargin)
...
% by Susan Whitfield-Gabriedli and Alfonso Nieto-Castanon. Can be
% potentially modified further to perform group level analysis by replacing
% the 1's in the index for subject with a subject index variable.
% 
% native_connectivity(P,S,M,ROI,R,V,save_conn...)
% 
% Inputs:
...
%
%   V: paths to vectors stored as cellstr. Alternatively, V can be a cell
%      array of structures / structure array that contains the following  
%      fields: 'names','onsets', and 'durations'. Assuming V{1} or V(1) 
%      corresponds to first session/block/run, and V{2} or V(2) corresponds
%      to the second session, etc. Also, assuming all sessions have
%      identical conditions; otherwise, will only analyze conditions that
%      are common to all sessions.
%
...
%       'overwrite': [true|false] overwriting existing extracted data.
%                     Default true.
...
% **************************  Analysis Details  ***************************
% 1). Basis condition: extract the time series based on conditions
%     specified by the vector (input V)
...
%
% ------------------------- Version History -------------------------------
% Created Edward Cui, January 20, 2014: for Resting State analysis
% Modified Edward Cui, April 30, 2014: Allowed input of vectors/multiple
% conditions for event related
% -------------------------------------------------------------------------
...
% Specify conditions
BATCH.Setup.conditions = ParseConditions(V);
% Set up covariates
...
function Conditions = ParseConditions(V)
% Parse input V
if iscellstr(V),V = cellfun(@load, V,'un',0);end
if isstruct(V),V = num2cell(V);end
% I. Pass in 'names' field
%make sure all sessions have identical fields and in the same order
names = cellfun(@(x) x.names,V,'un',0);
    function B = recur_cellstr_intersect(A)
        % function that recursively taking intersects for cellstrs
        B = intersect(A{1},A{2});
        if numel(A)>2, A=[{B},A(3:end)];B=recur_cellstr_intersect(A);end
    end
Conditions.names = recur_cellstr_intersect(names);
%the condition names is sorted. Get the reordered indices for each session
IND = cellfun(@(K) cell2mat(cellfun(@(x) find(ismember(K,x)),...
    Conditions.names,'un',0)),names,'un',0);
clear names;
% II. Pass in 'onsets' and 'durations' field
onsets = cellfun(@(x) x.onsets,V,'un',0);
durations = cellfun(@(x) x.durations,V,'un',0);
for nsess = 1:numel(V)
    for ncond = 1:numel(Conditions.names)
        Conditions.onsets{ncond}{1}{nsess} = onsets{nsess}{IND{nsess}(ncond)};
        Conditions.durations{ncond}{1}{nsess} = durations{nsess}{IND{nsess}(ncond)};
    end
end
