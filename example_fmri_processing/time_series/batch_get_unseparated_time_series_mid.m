...
save_dir = '/hsgs/projects/jhyoon1/midbrain_pilots/mid/analysis/EffectiveConnectivity/ROI_timeseries/';
...
for s = 1:length(subjects)
    ...
    for r = 1:length(ROIs)
        ROI_img = spm_vol(fullfile(ROI_dir,[subjects{s},ROIs{r}]));
        [X,Y,Z] = ind2sub(size(double(ROI_img.private.dat)),find(double()));
        ROI_ind{r} = [X(:)';Y(:)';Z(:)'];
        clear ROI_img X Y Z;
    end
    ...
    cell2csv(fullfile(save_dir,[subjects{s},'_timeseries.csv']),worksheet,',','a+');
end
