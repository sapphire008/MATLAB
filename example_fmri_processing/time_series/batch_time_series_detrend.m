...
for n = 1:length(subjects);
    ...
    [all_vox_time_series, mean_time_series] = time_series_extraction_with_trend(filtered_data, trend, onset_index, block_index, number_of_scans_in_series);
    
    for k = 1:size(all_vox_time_series,1)
        time_series(k,:) = mean(squeeze(all_vox_time_series(k,:,:)));
    end
    
    percent_change = (mean(time_series)/mean_trend) * 100;
