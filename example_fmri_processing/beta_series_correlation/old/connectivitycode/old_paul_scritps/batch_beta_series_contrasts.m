...
subject_dir = '/nfs/uhr08/conn_analysis_01_2009/subjects/';
...
img_prefix = 'R_atanh_corr_rDLPFC_L_7Vox_Jong_xyz_bin_LEFT';
...
for i = 1:length(all_subs)
    files = dir([subject_dir, '/', all_subs{i}, '/', subject_SPMmat_path, '/', img_prefix, '*']);
    ...
    if ~(length(files) == 2)
        ...
        for n = 1:length(files)
            file_list{n} = [subject_dir, '/', all_subs{i}, '/', subject_SPMmat_path, '/', files(n).name];
        end

        file_list = char(file_list);
%The structure after file_list specifies the contrast vector
        beta_series_contrasts(file_list,[-1 1],[subject_dir, '/', all_subs{i}, '/', subject_SPMmat_path, '/',output_prefix,'_contrast.nii']);
    end
end
