...
subject_dir = '/nfs/sn_loc/analysis/beta_series_correlations/8mm/';


% subject_SPMmat_path is the path in the subject's folders where the
% SPM.mat is located (in example above, 'conn/')
subject_SPMmat_path = '/nfs/sn_loc/analysis/model_estimation_trialwise_8mm/';
...
img_prefix = {'SNleftCue','SNleftDelay','SNleftProbe','STNleftCue','STNleftDelay','STNleftProbe','RNleftCue','RNleftDelay','RNleftProbe'};
corr_prefix = {'Zcorr','Rcorr','R_atanh_corr'};
...
for k = 1:length(corr_prefix)
for j = 1:length(img_prefix)
for i = 1:length(all_subs)
    files = dir([subject_dir, all_subs{i}, '/' corr_prefix{k} '_' img_prefix{j}, '*']);
    ...
    if ~(length(files) == 2)
        ...
        beta_series_contrasts(file_list,[-1 1],[subject_dir, all_subs{i}, '/' corr_prefix{k} '_' img_prefix{j} output_prefix,'.nii']);
    end
end
end
end
