...
subject_dir = '/nfs/sn_loc/analysis/beta_series_correlations/STNleft/';


% subject_SPMmat_path is the path in the subject's folders where the
% SPM.mat is located (in example above, 'conn/')
subject_SPMmat_path = '/nfs/sn_loc/analysis/model_estimation_trialwise_resliced_resampled/';
...
% e.g.: cueB-CueA
output_prefix = 'Zcorr_STNleftCueRed-DelayGreen';
...
for i = 1:length(all_subs)
    files = dir([subject_dir, all_subs{i}, '/', all_subs{i} '_Zcorr_' all_subs{i} img_prefix, '*']);
    ...
    if ~(length(files) == 2)
        ...
        beta_series_contrasts(file_list,[-1 1],[subject_dir, all_subs{i}, '/', all_subs{i} '_' output_prefix,'_contrast.nii']);
    end
end
