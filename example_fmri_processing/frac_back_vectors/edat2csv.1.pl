...
# An usage example would be "./edat2csv.pl foo bar". 
# Where "foo" is the name of the edat file to be processed, and
# "bar" is the column header in a string, delimited by comma ",".
# e.g. bar = 'Trial,Condition,Accuracy'.
# Make sure no space in the string.
#
# In MATLAB, use: perl('./edat2csv.pl', foo, bar)
#
...
# 
# However, the current script will look for a second argument for column 
# headers to replace @outheaders argument (EC062013)
#
...
if( $len < 2 ) {die "Need to specify column header\n"};

# get column header from argument
#@outheaders = qw(Running[Trial]);
$col_header = $ARGV[1];
@outheaders = split(/,/,$col_header);
