function [all_vox_time_series, mean_time_series] = ...
    time_series_extraction(D, onset_index, block_index, number_of_scans_in_series)
...
% all_vox_data: [num_events x num_voxels x num_time_points]
j = 1;
for n = 1:length(onset_index)%block
    for k = 1:length(onset_index(n).index),%trial within block
        try
            start_idx = onset_index(n).index(k);
            end_idx = onset_index(n).index(k)+number_of_scans_in_series-1;
            if block_index{n}(end) >= end_idx,
                all_vox_time_series(j,:,:) = D(start_idx:end_idx,:)';
                mean_time_series(j,:) = mean(all_vox_time_series(j,:,:),2);
                % Remove detrending
                % mean_time_series(j,:) = mean_time_series(j,:) - mean_time_series(j,1);
                j = j + 1;
            else
                disp(['1The time series in block ', num2str(n), ' and index ', num2str(onset_index(n).index(k)), ' failed']);
            end
        catch
            disp(['2The time series in block ', num2str(n), ' and index ', num2str(onset_index(n).index(k)), ' failed']);
        end
    end
end