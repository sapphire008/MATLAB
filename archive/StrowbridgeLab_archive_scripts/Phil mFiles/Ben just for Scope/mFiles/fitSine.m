function stringData = fitSine(yData, timePerPoint, startingTime, axisHandle)
...
%     phaseVal = unwrap(atan2(sinComp, cosComp)) - pi/2;
    ...
    line('parent', axisHandle, 'color', [1 0 0], 'buttondownfcn', ['set(get(get(gcbo, ''parent''), ''userdata''), ''string'', ''Tau = ' sprintf('%4.1f', estimates(2) * timePerPoint) ''')'],  'xData', startingTime + xData*1000, 'ydata', estimates(1) + estimates(2)*sin(estimates(3) + 2*pi*frequency*xData));
%     line('parent', axisHandle, 'color', [1 0 0], 'buttondownfcn', ['set(get(get(gcbo, ''parent''), ''userdata''), ''string'', ''Tau = ' sprintf('%4.1f', ampVal * timePerPoint) ''')'],  'xData', startingTime + xData*1000, 'ydata', -ampVal/2*sin(phaseVal + 2*pi*frequency*xData)+mean(yData));
