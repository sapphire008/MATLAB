...
%     end
    barSize = round((xGap/(5*10^howManyXDigits)));
    switch barSize
        case 3
            barSize = 2;
        case {4,6,7}
        ...
        case {8,9}
            barSize = 1;
            howManyXDigits = howManyXDigits + 1;
    end
    xBarSize = barSize*10^howManyXDigits; 

    yGap = diff(get(currentAxis, 'ylim'));
    howManyYDigits = floor(log10(yGap /5));
    barSize = round((yGap/(5*10^howManyYDigits)));
    switch barSize
        case 3
            barSize = 2;
        case {4,6,7}
        ...
        case {8,9}
            barSize = 1;
            howManyYDigits = howManyYDigits + 1;
    end    
%     if howManyYDigits <= 0
%         howManyYDigits = floor(log10(yGap / 2.5));
%     end        
    yBarSize = barSize*10^howManyYDigits; 
