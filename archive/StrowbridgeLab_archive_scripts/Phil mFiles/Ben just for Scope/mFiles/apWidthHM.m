        ...
        if ~nargout || (nargin == 4 && ishandle(axisHandle))
            ...
            try
                for i = 1:size(outData, 1)
                    % plot starts
                    line((riseLoc - 2) * timePerPoint + startTime, riseVal, 'color', [1 0 0], 'linestyle', 'none', 'marker', '+', 'markersize', 12, 'buttondownfcn', ['set(get(get(gcbo, ''parent''), ''userdata''), ''string'', ''AP WidthHM = ' sprintf('%6.3f', outData(i) * timePerPoint) ' msec'')']);
                    % plot stops
                    line((fallLoc - 2) * timePerPoint + startTime, fallVal, 'color', [0 0 0], 'linestyle', 'none', 'marker', '+', 'markersize', 12, 'buttondownfcn', ['set(get(get(gcbo, ''parent''), ''userdata''), ''string'', ''AP WidthHM = ' sprintf('%6.3f', outData(i) * timePerPoint) ' msec'')']);
                end
            catch
