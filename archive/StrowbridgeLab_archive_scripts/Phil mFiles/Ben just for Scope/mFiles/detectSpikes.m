function spikes = detectSpikes(inData, spikeHeight, whereCall)
...
if nargout == 0 && numel(spikes) > 0
    newScope(inData, 1:length(inData), 'Trace V');
    line(spikes, inData(spikes), 'Color', [1 0 0], 'LineStyle', 'none', 'Marker', '+');
    line((1:length(dataDerivative)) + 0.5, dataDerivative + mean(inData), 'color', [1 0 0]);
    line(dataSecDerivative + mean(inData), 'color', [1 1 0]);
    line([1 length(inData)], [spikeHeight spikeHeight], 'color', [0 1 0]);
end