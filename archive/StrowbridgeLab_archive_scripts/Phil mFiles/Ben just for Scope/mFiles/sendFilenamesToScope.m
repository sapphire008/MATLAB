function sendFilenamesToScope(commaList)
if isempty(commaList)
    msgbox ('file list string is empty');
    return
end
commas = [0 find(commaList == ',') length(commaList) + 1];
fileNames = {};
for i = 1:numel(commas) - 1
    if exist(strtrim(commaList(commas(i) + 1:commas(i + 1) - 1)), 'file')
...
%                  zData.protocol.channelNames{1} = 'Happy1';
%                  zData.protocol.channelNames{2} = 'Happy2';
%                   zData.protocol.channelNames{3} = 'Happy3';
...
%             for i = getappdata(0, 'scopes')'
                newScope(zData.traceData, zData.protocol, 4043);
                set(i, 'name', episodeName);
%             end
            if isappdata(0, 'benProtocolViewer')
                benProtocolViewer(commaList);
            end
...
%         msgbox('Error loading files in sendFilenamesToScope.m');
