function printWithScaleBars(figure)
...
% function for printing a newScope
    ...
    if ispref('newScope', 'exportSettings')
        tempPref = [getappdata(0, 'ExportWidth') getappdata(0, 'ExportHeight') getappdata(0, 'ExportWithOffset') 1];        
    end  
    ...
    if ispref('newScope', 'exportSettings') && tempPref(3)   
        if isappdata(figure, 'extraPrintText')
            tempHandle = annotation('textbox', [0 0 1 .1], 'linestyle', 'none', 'interpreter', 'none', 'verticalAlignment', 'bottom', 'horizontalAlignment', 'left', 'fontsize', 8, 'string', [get(figure, 'name') ' ' getappdata(figure, 'extraPrintText')]);    
        else
