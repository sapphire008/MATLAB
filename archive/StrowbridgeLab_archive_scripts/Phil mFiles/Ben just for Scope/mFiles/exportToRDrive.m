function exportToRDrive(figure)
...
% function for converting a newScope to an enhanced meta-file on the clipboard
    ...
    tempPref = [getappdata(0, 'ExportWidth') getappdata(0, 'ExportHeight') getappdata(0, 'ExportOffset') 1];        
    ...
    if ispref('newScope', 'exportSettings') && tempPref(3)
        tempHandle = annotation('textbox', [.1 .1 .8 .3], 'linestyle', 'none', 'interpreter', 'none', 'verticalAlignment', 'bottom', 'horizontalAlignment', 'left', 'fontsize', 1, 'string', evaluateExportText);
    end
...
%     print('-dmeta', '-noui', figure)
    print('-depsc2', '-r4800', '-noui', 'R:\mPhys2.eps', figure)
