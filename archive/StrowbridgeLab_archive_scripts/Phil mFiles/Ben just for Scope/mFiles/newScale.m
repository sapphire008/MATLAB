function newScale(objectHandle, eventInfo)
    ...
    for whichAxis = whichAxes
        ...
        switch get(handles.channelControl(whichAxis).scaleType, 'value')
            ...
            case 3 % float with an all-points histogram
                whichLine = get(handles.axes(whichAxis), 'child');
                whichLine = whichLine(end - 2);
            ...
            case 5 % float based on the min/max
                whichLine = get(handles.axes(whichAxis), 'child');
                whichLine = whichLine(strcmp(get(whichLine, 'userData'), 'data'));
                xData = get(whichLine, 'xData');
                yData = get(whichLine, 'yData');
                if iscell(xData)
                    maxVal = -100000;
            ...
            case 6 % defaults
                ...
                if channelNames{get(handles.channelControl(whichAxis).channel, 'value')}(end) == 'V'
                    set(handles.channelControl(whichAxis).maxVal, 'string', '40'); % hard-coded minimum and maximum values for channel display
                    set(handles.channelControl(whichAxis).minVal, 'string', '-100');
                else
                    set(handles.channelControl(whichAxis).maxVal, 'string', '500');
                    set(handles.channelControl(whichAxis).minVal, 'string', '-500');
                end
                set(handles.axes(whichAxis), 'ylim', [str2double(get(handles.channelControl(whichAxis).minVal, 'string')) str2double(get(handles.channelControl(whichAxis).maxVal, 'string'))]); 
                set(handles.channelControl(whichAxis).minVal, 'enable', 'off');
                set(handles.channelControl(whichAxis).maxVal, 'enable', 'off');                
    ...
    if strcmp(get(objectHandle, 'type'), 'figure')
        ...
        if xBounds(1) == handles.minX && zoomFactor == 1
            ...
            set(handles.slider, 'value', (xBounds(1) - handles.minX) / (handles.maxX - handles.minX) / (1- 1 / zoomFactor));
        end    
