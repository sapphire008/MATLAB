...
if ~nargin
	...
	xData = handles.minX:handles.xStep:handles.maxX;    
    if isempty(filterLength)
        % suggest a window that has, on average, 5 events in it
    	filterLength = round(length(xData) / length(events(1).data) * handles.xStep * 2.5) * 2;
    ...
    filterLength = round(str2double(tempData) / handles.xStep);
    ...
    for i = 1:numel(fromEvents)
        if numel(fromEvents(i).data) > 1
            ...
            whereData = [round((fromEvents(i).data) / handles.xStep - filterLength / 2) round((fromEvents(i).data) / handles.xStep) + filterLength / 2 + 1];
            [whereData indices] = sort(whereData);           
            lastSum = sum(whereData <= 0);
            yData(i, 1:whereData(lastSum + 1)) = lastSum / (filterLength * handles.xStep / 1000);
            for j = lastSum + 1:min([find(whereData < length(yData), 1, 'last') - 1 size(whereData, 2) - 1]);
                lastSum = lastSum + changeData(indices(j));
                yData(i, whereData(j):whereData(j + 1)) = lastSum / (filterLength * handles.xStep / 1000);
            end
        end
        if numel(toEvents(i).data) > 1
            ...
            changeData(end/2 + 1:end) = -1;
            whereData = [round((toEvents(i).data) / handles.xStep - filterLength / 2) round((toEvents(i).data) / handles.xStep) + filterLength / 2 + 1];
            [whereData indices] = sort(whereData);           
            lastSum = sum(whereData <= 0);
            yData(i, 1:whereData(lastSum + 1)) = lastSum / (filterLength * handles.xStep / 1000);
            for j = lastSum + 1:min([find(whereData < length(yData), 1, 'last') - 1 size(whereData, 2) - 1]);
    ...
    changeData(end/2 + 1:end) = -1;
    whereData = [round((allCoincidents) / handles.xStep - filterLength / 2) round((allCoincidents) / handles.xStep) + filterLength / 2 + 1];
    [whereData indices] = sort(whereData);           
    lastSum = sum(whereData <= 0);
    zData = zeros(1, size(xData, 2));    
    zData(1:whereData(lastSum + 1)) = lastSum / (filterLength * handles.xStep / 1000);
    for j = lastSum + 1:min([find(whereData < length(yData), 1, 'last') - 1 size(whereData, 2) - 1]);
        lastSum = lastSum + changeData(indices(j));
        zData(whereData(j):whereData(j + 1)) = lastSum / (filterLength * handles.xStep / 1000);
    end
    ...
    filterLength = filterLength * handles.xStep;    