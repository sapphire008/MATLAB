...
if showDisplay
    ...
    function errorSum = minFun(params)
        ...
        errorSum = sum(distanceFunction(([sign(params(1:end - 1) * (centroidB - centroidA)') .* params(1:end - 1) params(end)] * setA') ./ normFactor)) ./ numel(setA) + sum(distanceFunction(([sign(params(1:end - 1) * (centroidB - centroidA)') .* params(1:end - 1) params(end)] * setB') ./ -normFactor)) ./ numel(setB);
