...
%
%   SUBPLOT(m,n,p,'inset', [left bottom right top]) creates an axis with
%   the specified offset (default is inset = [.2 .18 .04 .1];
%
...
%   $Revision: 5.22.4.14 $  $Date: 2005/12/15 20:53:51 $
...
elseif narg >= 4
  ...
  elseif strcmpi(arg,'inset')
    % passed in subplot(m,n,p, 'inset', [left bottom right top])
    if size(varargin{2}) == [1 4]
        inset = varargin{2};
    else
        error('Inset must be of the form [left bottom right top]')
    end
  else
...
if create_axis
  ...
  if (useOuterPosition)
    ax = axes('units','normal','OuterPosition',outerpos,...
              'LooseInset',loose,'Parent',parent,pvpairs{:});
  else
    ax = axes('units','normal','Position',position,...
              'Parent',parent,pvpairs{:});
  end
  set(ax,'units',get(parentfigure,'defaultaxesunits'))
  if useAutoLayout
    addAxesToGrid(ax,nrows,ncols,row,col,inset);
  end
elseif move_axis
  ax = handle;
