function [header d]=abfload(fn,varargin)
...
% changes made by Phil Larimer Jan 2009
% can get full header info out, including channelNames, timePerPoint,
% fileName, startingValues, ampEnable, cellTime, drugTime, episodeTime
% 
% minimum protocol includes:
% ampEnable, startingValues, channelNames, sweepWindow, fileName,
% timePerPoint
%
% Changes:
% order of output variables
% encompassed data reading section in an if nargout > 1 block
% added part 2e
% add startingValues after data read section
% changed verbose to 0
% tucked the 'opening' display text into an 'if verbose' block
...
% -------------------------------------------------------------------------
%    PART 2e: add parameters to header for output
% -------------------------------------------------------------------------
header.fileName = fn;
header.timePerPoint = ProtocolSec.fADCSequenceInterval;
for chIndex = 1:size(recChNames, 1)
    header.channelNames{chIndex} = strtrim(recChNames(chIndex,:));
end
header.startingValues = nan(size(header.channelNames)); % this is overwritten after data is read
if lActualEpisodes
    header.sweepWindow = ProtocolSec.lNumSamplesPerEpisode * fADCSampleInterval / 1000;
else
    header.sweepWindow = lActualAcqLength * fADCSampleInterval / 1000;
end
header.cellTime = lFileStartTime / 1000; % assuming this is in ms since midnight
header.fileDate = [fix(uFileStartDate/10000)...  % this is a datevector
    fix(mod(uFileStartDate, 10000) / 100)... % use the datestr function to turn it into text
    mod(uFileStartDate, 100)...
    fix(lFileStartTime / (60 * 60 *1000))...
    fix((lFileStartTime - fix(lFileStartTime / (60 * 60 *1000)) * (60 * 60 * 1000)) / (60 * 1000))...
    fix((lFileStartTime - fix(lFileStartTime / (60 * 60 *1000)) * (60 * 60 * 1000) - fix((lFileStartTime - fix(lFileStartTime / (60 * 60 *1000)) * (60 * 60 * 1000)) / (60 * 1000)) * (60 * 1000)) / 1000)];
header.numEpisodes = lActualEpisodes;
whichAmps = unique(cellfun(@(x) str2double(x(3)), header.channelNames));
for i = 1:max(whichAmps)
    if ismember(i, whichAmps)
        header.ampEnable{i} = 1;
    else
        header.ampEnable{i} = 0;
    end
end
...
if nargout > 1
    ...
    for i = 1:size(d, 3)
        header(i) = header(1);
        header(i).startingValues = mean(d(1:10, :, i), 1);
        if size(d, 3) > 1
            header(i).fileName = [fn '.E' sprintf('%0.0f', i)];
        end
    end
end
fclose(fid);
