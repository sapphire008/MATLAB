...
if channelType(1) == 'V'
    % curent clamp
    channelNum = find(cellfun(@(x) ~isempty(x), strfind(protocol.channelNames, ['Vm' char(48 + ampNum)])), 1);
elseif channelType(1) == 'I'
    % voltage clamp
    channelNum = find(cellfun(@(x) ~isempty(x), strfind(protocol.channelNames, ['Im ' char(48 + ampNum)])), 1);
