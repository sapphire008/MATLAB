function numEpisodes = addEpisodes(nodeInfo)
    ...
    fileNames = dir(path);
    fileNames = {fileNames(cellfun(@(x) numel(x) > 2 && strcmpi(x(end - 2:end), 'mat'), {fileNames.name}) & ~[fileNames.isdir]).name};    
    if exist(path, 'file') == 2
        path = path(1:find(path == filesep, 1, 'last'));
    end
    
    feval(getappdata(getappdata(0, 'fileBrowser'), 'populateColumns'), handles{4}, 'pjp');
...
%     handles{4}.setName('Separate');
    ...
    while i <= length(fileNames)
        header = readPJP([path fileNames{i}], 1);
        if ~isempty(header)
