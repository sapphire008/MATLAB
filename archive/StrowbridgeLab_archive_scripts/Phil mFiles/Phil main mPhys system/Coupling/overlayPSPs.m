...
% display the PSPs for a given trace overlayed
    ...
    for epiIndex = 1:numel(startTimes)
        if ~isempty(startTimes{epiIndex})
            startTimes{epiIndex} = startTimes{epiIndex}(startTimes{epiIndex}(:,3) > - timeWindow(1) * 1000 / timePerPoint + 1 & startTimes{epiIndex}(:,3) < length(inData) / 1000 * timePerPoint - timeWindow(2), 3);
