function changeCurrent(ampNum, figHandle)
...
% get amplifier output types
    ...
    currentChannels = getpref('amplifiers', 'currentChannels');
...
% determine which amp if none was passed
    if nargin < 1
        ampNum = find(protocolHandles.ampCurrent == gcbo);
    end
    
if isfield(protocolHandles, 'channelType')

	lastChannel = get(protocolHandles.ampCurrent(ampNum), 'userData');
%     callStack = dbstack;
% 	if lastChannel == get(protocolHandles.ampCurrent(ampNum), 'value') && ~strcmp(callStack(end).name, 'changeAmp')
% 		return
% 	end	

	% is the value negative
	if get(protocolHandles.ampCurrent(ampNum), 'value') > 0 && get(protocolHandles.ampCurrent(ampNum), 'value') < adBoard.numRead + 1 % not in hardware mode or channel disabled
    % check to make sure the channel isn't already in use
		% determine which amp is using this channel
		if any(cell2mat(get(protocolHandles.ampVoltage, 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value')) 
			oldAmp = find(cell2mat(get(protocolHandles.ampVoltage, 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value'));
			if strcmp(questdlg(['Channel ' num2str(get(protocolHandles.ampVoltage(ampNum), 'value')) ' is currently being used for voltage channel of amp ' char(oldAmp + 64)  '.  Setting this channel to be the voltage channel for amp ' char(ampNum + 64) ' will set the voltage channel of amp ' char(oldAmp + 64) ' to none.'], '', 'Do it', 'Cancel', 'Do it'), 'Do it')
				...
				set(protocolHandles.ampCurrent(ampNum), 'value', lastChannel);
				return
			end
		elseif any(cell2mat(get(protocolHandles.ampCurrent([1:ampNum - 1 ampNum + 1:end]), 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value')) 
			oldAmp = find(cell2mat(get(protocolHandles.ampCurrent([1:ampNum - 1 ampNum + 1:end]), 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value'));
			if oldAmp > ampNum
				oldAmp = oldAmp + 1;
			end			
		...
		elseif any(cell2mat(get(protocolHandles.ampTelegraph, 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value')) 
			oldAmp = find(cell2mat(get(protocolHandles.ampTelegraph, 'value')) == get(protocolHandles.ampCurrent(ampNum) ,'value'));
    ...
    % set the choices 
        set(protocolHandles.channelType(get(protocolHandles.ampCurrent(ampNum), 'value')),...
            'string', currentChannels(get(protocolHandles.ampType(ampNum), 'value'), :),...
            'userData', currentFactors(get(protocolHandles.ampType(ampNum), 'value'), :));
        setappdata(protocolHandles.channelType(get(protocolHandles.ampCurrent(ampNum), 'value')), 'source', ['Amp ' char(64 + ampNum) ', ']);
	...
	if ~strcmp(get(gcbo, 'tag'), 'ampType') || (strcmp(get(gcbo, 'tag'), 'ampType') && get(gcbo, 'value') == 1) || get(protocolHandles.ampCurrent(ampNum), 'value') < 0
        if lastChannel ~= get(protocolHandles.ampCurrent(ampNum), 'value') && lastChannel < adBoard.numRead + 1
		...
		if get(protocolHandles.ampCurrent(ampNum), 'value') > 0
			set(protocolHandles.ampCurrent(ampNum), 'userData', get(protocolHandles.ampCurrent(ampNum), 'value'));
		end
	end
