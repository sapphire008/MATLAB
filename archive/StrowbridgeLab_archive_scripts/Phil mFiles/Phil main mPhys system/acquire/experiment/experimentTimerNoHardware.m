 function experimentTimerNoHardware(varargin)
                ...
                % determine what channels need to be recorded from or stimulated
                    ...
                    if ~isempty(whichAmpsTp)                       
                        ...
                        % generate the stimulus data
                            ...
                            stimData = repmat(int32(biasCurrent), 2 * sweepTime * 1000 / protocolData.timePerPoint + 3, 1); 
                        ...
                        % check to make sure we have the data that we need
                            ...
                            elseif protocolData.ampStimulus{whichAmpSealTest} > 0 && protocolData.ampStimulus{whichAmpSealTest} == 5
                                ...
                                if ismember(protocolData.ampStimulus{whichAmpSealTest}, writeInstructions)
                                    stimData(startTime * 1000 / protocolData.timePerPoint:stopTime * 1000 / protocolData.timePerPoint, find(writeInstructions == protocolData.ampStimulus{whichAmpSealTest})) = stimData(startTime * 1000 / protocolData.timePerPoint:stopTime * 1000 / protocolData.timePerPoint, find(writeInstructions == protocolData.ampStimulus{whichAmpSealTest})) + stepSize; %#ok<FNDSB>
                                else
                                    stimData(:, end + 1) = [zeros(startTime * 1000 / protocolData.timePerPoint - 1, 1); ones((stopTime - startTime) * 1000 / protocolData.timePerPoint + 1, 1) * stepSize; zeros((sweepTime - stopTime) * 1000 / protocolData.timePerPoint + 3, 1)];    
...
%                     end
                ...
                if ~isempty(varargin{2})
					...
					for i = 1:size(varargin{2}, 2)
						if protocolData.ampTpEnable{whichDAC(i)}
							analogStim(:,i) = (varargin{2}(:,i) + biasCurrent(find(cell2mat(protocolData.ampTpEnable)) == whichDAC(i)));
                        else
                            analogStim(:,i) = varargin{2}(:,i);                            
						end
					end
				else
					analogStim = [];
		...
		elseif ischar(varargin{1}) && strcmp(varargin{1}, '-extendEpisode')
            ...
            % preallocate space for traceData
                ...
                memDetails = memory;
                traceData = nan(min([100000000 round(memDetails.MaxPossibleArrayBytes / 9 / numChannels) - numel(traceData)]), numChannels);                                           traceData(1:numSamples, :) = tempTraceData;
			return;
		else
...
% create instructions sequence
    ...
    if streamingEpisode && ~isempty(lastStims) && size(stimData, 1) < 300000 / protocol.timePerPoint
		...
		if ~isempty(savedStim)
			savedStim(end + 1:end + 1000000 / protocolData.timePerPoint, :) = lastStims(ones(1000000 / protocol.timePerPoint, 1), 1:size(lastStims, 2));
		end
    end
    if numTrimmed > 0
