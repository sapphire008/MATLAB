function enrichmentData = metaCorrEvents2(varargin)
persistent correlationBounds
persistent binWidth
    
if nargin == 0
    enrichmentData = 'Metacorrelate [-1 1]';
    return
end
    ...
    for toAxis = toIndex
        ...
        for i = 1:numel(toEvents)
            autoCorrs{toAxis}(i,:) = hist(crossCorr([toEvents(i).data' toEvents(i).data'], correlationBounds + [-1 1] * (binWidth / 2)), binLocs);
        end
    end
    numEvents = 0;
    numCount = 0;
                ...
                for i = 1:numel(toEvents)
                    ...
                    for j = 1:numel(fromEvents)
                        ...
                        if any(fromEventsSame == j)
                            count = count + hist(spikeTimes, binLocs) ./ sqrt(autoCorrs{toAxis}(i,:) .* autoCorrs{fromAxis}(j,:));        
                            numCount = numCount + 1;
                        else
                            scCount = scCount + hist(spikeTimes, binLocs) ./ sqrt(autoCorrs{toAxis}(i,:) .* autoCorrs{fromAxis}(j,:));
    ...
    figure
    subplot(5,1,5)
    bar(binLocs, scCount./numScCount - 1);
    set(gca, 'ylim', [-1 1]);
    xlabel('Time (ms)');
    ylabel('Shuffled')
    subplot(5,1,4)
    bar(binLocs, count./numCount - 1);
    set(gca, 'ylim', [-1 1]);    
    ylabel('Raw')    
    subplot(5,1,3);
    bar(binLocs, count ./ numCount - scCount./numScCount);
    set(gca, 'ylim', [-1 1]);    
    ylabel('Correlation');
    subplot(5,1,1:2);    
    bar(binLocs, 100 .* (count ./ numCount) ./ (scCount./numScCount) - 100);
    ylabel('% Increase');   
    return
