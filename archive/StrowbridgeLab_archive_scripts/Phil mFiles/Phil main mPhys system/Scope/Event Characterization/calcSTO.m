function outText = calcSTO(varargin)
...
if ~nargin
    ...
    if numel(events(varargin{5}).data)
        ...
        eventTriggeredAverage(events(varargin{5}).data' ./ handleList.xStep(get(handleList.channelControl(handleList.axes == gca).channel, 'value')), data, windowBounds, handleList.xStep(cell2mat(get([handleList.channelControl.channel], 'value'))), 1);  
    end  