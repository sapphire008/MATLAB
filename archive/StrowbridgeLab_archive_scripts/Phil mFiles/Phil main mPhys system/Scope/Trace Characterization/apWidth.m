...
if ~nargin
    ...
    if length(spikes) > 1
		...
		for spikeIndex = 1:length(spikes) - 1
            ...
            loc(spikeIndex) = find(traceData(spikes(spikeIndex) + 2 + find((traceData(spikes(spikeIndex) + (0:25)) - traceData(spikes(spikeIndex))) > 20, 1, 'first'):min([spikes(spikeIndex + 1) spikes(spikeIndex) + 100])) - traceData(spikes(spikeIndex)) < 0, 1, 'first') - 1;
            loc(spikeIndex) = loc(spikeIndex) + find((traceData(spikes(spikeIndex) + (0:25)) - traceData(spikes(spikeIndex))) > 20, 1, 'first');
            ...
            pointDrop(spikeIndex) = traceData(spikes(spikeIndex) + loc(spikeIndex)) - traceData(spikes(spikeIndex) + 1 + loc(spikeIndex)); 
            outData(spikeIndex) = (loc(spikeIndex) - ((traceData(spikes(spikeIndex)) - traceData(spikes(spikeIndex) + 1 + loc(spikeIndex))) / pointDrop(spikeIndex)) + 1);
		end
