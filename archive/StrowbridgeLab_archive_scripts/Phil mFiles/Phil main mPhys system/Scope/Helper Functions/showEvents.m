function showEvents(axisHandle)
    ...
    if strcmp(get(findobj(ancestor(axisHandle, 'figure'), 'Label', 'Events'), 'checked'), 'off')
        return
    end
    ...
    locMen = uicontextmenu('parent', ancestor(axisHandle, 'figure'));
        ...
        uimenu(locMen, 'Label', 'Export Series to Clipboard', 'callback', {@exportSeriesClipboard, 0}, 'Separator', 'on');
...
function setMenu(varargin)
    ...
    switch get(gcf, 'selectionType')
        ...
        case 'normal' % display the data value of the point
            ...
            xCoord = round(((pointerLoc(1) - 7) / (figureLoc(3) - 55)  * diff(get(handles.axes(1),'Xlim')) + min(get(handles.axes(1),'Xlim'))) / handles.xStep(get(handles.channelControl(handles.axes == gca).channel, 'value'))) * handles.xStep(get(handles.channelControl(handles.axes == gca).channel, 'value'));
...
function exportSeries(varargin)
    ...
    if any(tempVarName == filesep)
        tempVarName = tempVarName(find(tempVarName == filesep, 1, 'last') + 1:end);
        tempVarName(tempVarName=='.' | tempVarName==' ') = '_';
    else
        tempVarName = tempVarName(1:find(tempVarName == ',', 1, 'first') - 1);
    end
    varName = inputdlg('Enter a name for the workspace variable', 'Export', 1, {genvarname(tempVarName)});
...
function exportSeriesClipboard(varargin)
    events = getappdata(varargin{4}, 'events');    
    clipText = '';
    events = num2str(events(varargin{5}).data');
    for i = 1:size(events, 1)
        clipText = [clipText events(i, :) char(13)];
    end
