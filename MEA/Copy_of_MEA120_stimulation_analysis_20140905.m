...
for n = reproc_ind
    ...
    spkt(1,n) = spkt(1,n)+400;%modified, original -0.5, now -0.1.
    spkt(2,n) = spkt(1,n)+1000;%modified, originally +18, now +1.2
    ...
    filt_X = filtfilt(b,1,X);
    save(fullfile(result_dir,[stim_channel_list{n},'.mat']),'MEA','X','filt_X','fs','dt','pitch','xy','nT','nch');
    X = filt_X; clear filt_X;
    ...
    fname = fullfile(result_dir,[fname,sprintf('_short_elec_%s.gif',stim_channel_list{n})]);
    ...
    fprintf('\n');
    save(fullfile(result_dir,[stim_channel_list{n},'.mat']),'Vmax','V','I','-append');
    ...
    [~,IND0] = min(abs(INDTIME));
    [~,IND1] = min(abs(INDTIME-0.5));
...
%% plot time series
    ...
    for t = 1:length(Tinterv)
        ...
        saveas(gcf,fullfile(result_dir,[stim_channel_list{n},'.fig']));
        pause(0.05);
    end
